...
def panel_lib_update():
    """
    after load erc wp
    pasre pdf erc wp to extract panels
    update data_files/panels.json with new info and new panels
    ...
    erc_panels = json.load(open(f"{PATH_HARVEST}/erc_panels.json", 'r', encoding='UTF-8'))

    base_codes = {p["panel_code"] for p in base}
    erc_lookup  = {p["panel_code"]: p for p in erc_panels}

    updated = []

    # Mise à jour et conservation
    for entry in base:
        code = entry["panel_code"]
        if code in erc_lookup:
            # erc écrase la base sur les champs existants
            updated.append({**entry, **erc_lookup[code]})
            print(f"  UPDATED: {code}")
        else:
            updated.append(entry)
            print(f"  KEPT:    {code}")

    # Ajout des nouveaux codes absents de la base
    for p in erc_panels:
        if p["panel_code"] not in base_codes:
            updated.append(p)
            print(f"  ADDED:   {p['panel_code']}")

    # Sauvegarde
    with open('data_files/panels.json', 'w', encoding='utf-8') as f:
        json.dump(updated, f, ensure_ascii=False, indent=4)

    print(f"\nTotal: {len(updated)} panels")

# table panels'''
def merged_panels(df):

    """"
    1. add panels ERC and MSCA
    2. panels exist for other programmes (ex: FET, Future and Emerging Technologies) but not in panels.json : export data for check and update panels.json
