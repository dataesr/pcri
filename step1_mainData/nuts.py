...
def nuts_lien(app1, part, lien):
    # from step1_mainData.nuts import nuts_department
        ...
        if len(nuts_a) == len(nuts_a.merge(pp_app, how='left', on=['project_id', 'generalPic', 'proposal_participant_pic'])):   
        ...
        if len(nuts_p) == len(nuts_p.merge(pp_part, how='left', on=['project_id', 'generalPic', 'calculated_pic'])):   
        ...
        lien['partNlien'] = lien.groupby(['project_id', 'proposal_orderNumber', 'generalPic', 'calculated_pic'], dropna = False).pipe(lambda x: x.orderNumber.transform('nunique'))
        lien['partNlien'] = lien.groupby(['project_id', 'orderNumber', 'generalPic', 'calculated_pic'], dropna = False).pipe(lambda x: x.proposal_orderNumber.transform('nunique'))
