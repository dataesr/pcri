...
from remote_process.grist import geoG, add_records_to_grist
...
def french_localisation(df):
    ...
    if not test.empty:
        ...
        print(f"- ⚠️ ! check {len(final_df)} new records added to Grist for French localisation")
...
def geoloc_foreign_back():
    ...
    if not geo2.empty:
        print(f"- ⚠️ ! check {len(geo2)} new records added to Grist for foreign localisation")
