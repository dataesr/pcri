...
def IDpaysage_successor(df):
    ...
    if len(paysage_successor)>0:    
        ...
        paysage = df.loc[df['source_id']=='paysage'].assign(id_clean=df['id_extend'])
...
def IDpaysage_name(paysage):
    ...
    for i in ['acronymEn', 'acronymLocal']:
        paysage_name['acronym'] = np.where(paysage_name.acronym.isnull(), paysage_name[i], paysage_name.acronym)
